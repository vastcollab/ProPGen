...
# non-random initialization to specific frequencies
...
if __name__ == '__main__':
    ...
    # initialize to specific frequency vector

    allowed_pairs = [(0, 0), (0, 1), (1, 0), (1, 1)]
    ...
    freqs = [0.4, 0.1, 0.05, 0.45]
    freqs = np.array(freqs, dtype=float)

    counts = (freqs * N).astype(int)
    # fix rounding to ensure sum == N
    while counts.sum() < N:
        counts[np.argmax(freqs - counts / N)] += 1

    initial_population = []
    for pair, count in zip(allowed_pairs, counts):
        initial_population.extend([pair] * count)

    # shuffle so they aren’t ordered
    np.random.shuffle(initial_population)
    indices = [allowed_pairs.index(pair) for pair in initial_population]
